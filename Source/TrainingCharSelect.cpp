...
void TrainingCharSelect::UpdateEvent(sf::Event event) {
}

void TrainingCharSelect::Update() {
	if (sceneDone)
		return;
...
bool TrainingCharSelect::CheckP1CharInputs() {
	int yAxis, xAxis;
...
bool TrainingCharSelect::CheckP1StageInputs() {
	int yAxis, xAxis;
...
bool TrainingCharSelect::CheckP2CharInputs() {
	int yAxis, xAxis;
	bool selectionChanged = false;
...
/*
bool TrainingCharSelect::CheckP2StageInputs() {
	int yAxis, xAxis;
	...
	if (p1Controls->isActionPressed(Controls::Actions::Special)) {
		if (!p2ConfirmHeld) {
			p2ConfirmHeld = true;
			OnToTheFight();
			return false;
...
void TrainingCharSelect::Draw(sf::RenderWindow* window) {
	if (sceneDone)
		return;
	...
	if (p1SupportSelected && p2SupportSelected) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if ((p1MainSelected && !p1SupportSelected) || (p2MainSelected && !p2SupportSelected)) supportFighters[i].DrawPortrait(window);
		}
...
void TrainingCharSelect::OnToTheFight() {
	...
	ApplicationData::sceneManager->SetCurrentScene(TRAINING);
	return;
}

int TrainingCharSelect::GetRandomPlayable(int playerNo) {
...
int TrainingCharSelect::GetRandomSupport(int playerNo) {
	string mainName = (2 == playerNo) ? p2Main.GetName() : p1Main.GetName();
	srand(time(NULL));
	int random = rand() % 3;
	if (1 == random)
...
void TrainingCharSelect::UpdateP1UI() {
...
void TrainingCharSelect::UpdateP2UI() {
